...
\begin{itemize}
    \item Soft corners in landscape photos indicating vignetting
    \item Color fringing around high-contrast edges showing chromatic aberration
    \item Curved lines in architectural photos revealing geometric distortion
...
The perceived sharpness of a lens manifests in how well it preserves fine details and edge clarity. While traditionally measured with expensive test charts, edge detection algorithms can quantify sharpness by analyzing high-contrast transitions in images. Key metrics include:
...
\begin{itemize}
    ...
    \item PSF(x) is the Point Spread Function of the lens
\end{itemize}

\subsubsection{Vignetting Measurement}
Corner darkening appears most prominently in evenly lit scenes. By photographing a uniformly illuminated surface, we can measure the relative light falloff from center to corners. The mathematical model follows the cos⁴ law:

I(r) = I₀ cos⁴(arctan(r/f))

Where:
\begin{itemize}
    \item I(r) is the illumination at radius r from center
    \item I₀ is the center illumination
...
\begin{itemize}
    \item Circularity ratio = 4πA/P², where A is area and P is perimeter
...
r_distorted = r(1 + k₁r² + k₂r⁴)

Where:
\begin{itemize}
    \item r is the radial distance from center
    \item k₁, k₂ are distortion coefficients
\end{itemize}
...
These mathematical frameworks translate into specific measurement protocols that balance precision with practicality in a DIY testing environment. Each protocol defines:
\begin{itemize}
    \item Required test conditions
    \item Capture parameters
    \item Analysis algorithms
